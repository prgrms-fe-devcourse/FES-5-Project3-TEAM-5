...
export async function saveAccountItem({
  ...
  if (repeatInstallmentData?.mode === '반복') {
    const { data, error } = await supabase
      ...
      .insert({
        frequency: mapPeriodToFrequency(
          repeatInstallmentData.selectedPeriod as string
        ), // RepeatInstallmentModal 모달에서 기본 값이 매일이어서 단언
        end_date: repeatInstallmentData.endDate
          ? repeatInstallmentData.endDate.toISOString()
          : null
      })
  ...
  const installmentAmount = Math.floor(
    amount / Number(repeatInstallmentData?.installment)
  )
  ...
  const { data, error } = await supabase
    ...
    .insert({
      amount:
        repeatInstallmentData?.mode === '할부' ? installmentAmount : amount,
      type,
      date,
      user_id: userId,
      group_id: groupId,
