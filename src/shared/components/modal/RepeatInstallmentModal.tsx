...
function RepeatInstallmentModal({ onClose }:Props) {
  ...
  // 토글 상태에 따라 값 변경
  const periods = isBiMonthly ? ["격일", "격주", "격월", "격년"] : ["매일", "매주", "매월", "매년"]
  ...
  return (
    <BaseModal onClose={onClose}>
      {/* 상단 탭 */}
      <div className="mt-5">
        <BinaryTabs value={tab} onChange={setTab} options={["반복", "할부"]} />
      </div>
      ...
      {tab === "할부" && (
        <div className="mt-5">
          <label className="block mb-2 text-gray-700">할부 개월 수</label>
          <input
            type="number"
            placeholder="할부 개월 수를 입력하세요 (예: 12)"
            className="w-full rounded-lg border border-gray-300 p-2 text-gray-700"
          />

          <div className="mt-9">
            <SubmitButton text="할부 설정" />
          </div>
        </div>
      )}
    </BaseModal>
